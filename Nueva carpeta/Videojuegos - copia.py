...
def obtener_numero(mensaje, tipo=int):
    """
    Solicita entrada al usuario y la convierte al tipo especificado.
    Retorna el valor convertido o None si hay error.
    """
    try:
...
def main():#Función principal con menú para la tienda de videojuegos
    ...
    while True:
        ...
        if opcion == "1":
            ...
            for i, juego in enumerate(juegos, 1): #Enumerate para mostrar el numero del juego junto con su información
                print(f"\n{i}. {juego.mostrar_info()}") 
            ...
            num_juego = obtener_numero("Selecciona el número del juego: ", int)
            if num_juego is not None:
                num_juego -= 1
                porcentaje = obtener_numero("Ingresa el porcentaje de descuento: ", float)
            ...
            num_usuario = obtener_numero("Selecciona el número del usuario: ", int)
            ...
            if num_usuario is not None:
                num_usuario -= 1
                monto = obtener_numero("Ingresa el monto a depositar: ", float)
